...
def diagnose():
    """診斷端點：測試每個模型，顯示 Z.ai 原始回應（部署後可刪除）"""
    results = []
    for model in ZAI_MODELS:
        try:
            r = requests.post(
            ...
            results.append({'model': model, 'status': r.status_code, 'response': r.text[:400]})
        except Exception as e:
            results.append({'model': model, 'status': 'error', 'response': str(e)})
    return jsonify({'endpoint': ZAI_BASE_URL, 'key_prefix': ZAI_API_KEY[:8] + '...', 'results': results})
