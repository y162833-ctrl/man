...
# glm-4-flash = 免費模型，速率限制寬鬆
# glm-4-plus  = 高級模型，需要額度（如遇429請換回flash）
...
def chat():
    try:
        ...
        # Call Z.ai API with retry logic (handles 429 rate limits)
        max_retries = 3
        last_error = None
        for attempt in range(max_retries):
            ...
            if response.status_code == 429:
                ...
                time.sleep(wait)
                last_error = '小Man 現在很忙，請稍等一下再試 🙏'
                continue

            if not response.ok:
                error_text = response.text
                print(f"Z.ai API error {response.status_code}: {error_text}")
        ...
        return jsonify({'error': last_error or '請求次數過多，請稍後再試 🙏'}), 429
...
def health():
    return jsonify({'status': 'ok', 'model': ZAI_MODEL})
