...
GEMINI_MODELS = ["gemini-flash-latest", "gemini-1.5-flash-latest"]
        ...
        for model in ZAI_MODELS:
            ...
            for attempt in range(2):
                ...
                if r.ok:
                    ...
                    ai_text = msg.get('content', '').strip()
                    # glm-5 是思考模型，content 可能為空（tokens 被 reasoning 用光）
                    # 備用：嘗試從 reasoning_content 取最後段落
                    if not ai_text:
                        reasoning = msg.get('reasoning_content', '').strip()
                        if reasoning:
                            # 取思考內容最後一段作為回覆
                            ai_text = reasoning.split('\n')[-1].strip() or reasoning[-300:]
                    if not ai_text:
                        print(f"  ⚠️ Z.ai {model} returned empty content, skipping")
                        break  # 試下一個 model
                    print(f"  ✅ Z.ai success with {model} ({len(ai_text)} chars)")
